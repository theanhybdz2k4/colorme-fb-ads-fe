...
export function AccountReportingPage() {
    ...
    const fetchReport = async () => {
        try {
            ...
            const { data: result, error: invokeError } = await supabase.functions.invoke(`ads-analytics-report/report/account/${id}`, {
                method: 'GET',
            });
        ...
        <div className="p-6 md:p-8 max-w-6xl mx-auto space-y-8 animate-in fade-in zoom-in-95 duration-500">
            ...
            <div className="flex items-center justify-between">
                <Button variant="ghost" asChild className="hover:bg-transparent px-0 text-muted-foreground hover:text-foreground">
                    <Link to={ROUTES.ACCOUNTS}><ArrowLeft className="w-4 h-4 mr-2" /> Trở về danh mục TKQC</Link>
                </Button>
