...
export function FlowSelect({ flows, selectedKey, onChange, compact, placeholder }: {
    ...
    const selectedFlow = flows.find(f => f.flow_key === selectedKey);
    ...
    return (
        <Popover open={open} onOpenChange={setOpen}>
            <PopoverTrigger asChild>
                <div 
                    className={cn(
                        "flex-1 flex flex-wrap gap-1 items-center p-1.5 rounded-lg border border-border bg-background cursor-pointer hover:border-primary/40 transition-colors empty:after:content-[attr(data-placeholder)] empty:after:text-muted-foreground empty:after:text-xs empty:after:py-1 empty:after:px-1 min-h-[38px]",
                ...
                >
                    {selectedKey ? (
                        ...
                        >
                            {selectedFlow?.display_name || selectedKey}
                            <button 
            ...
            <PopoverContent className="w-[320px] p-0 shadow-xl border border-border overflow-hidden" align="start">
                ...
                <div className="max-h-[320px] overflow-y-auto p-1 custom-scrollbar">
                    {showCustomAdd && (
                        <div
                            onClick={() => { select(search.trim()); setSearch(''); }}
                    ...
                    {filtered.map(f => (
                        <div
                            key={f.flow_key}
                            onClick={() => { select(f.flow_key); setSearch(''); }}
...
export function FlowMultiSelect({
    ...
    const showCustomAdd = search.trim() !== '' && !flows.some(f => f.flow_key === search.trim());
            ...
            <PopoverContent className="w-[320px] p-0 shadow-xl border border-border overflow-hidden" align="start">
                ...
                <div className="max-h-[320px] overflow-y-auto p-1 custom-scrollbar">
                    ...
                    <div className="grid grid-cols-1 gap-0.5">
                        {filtered.map(f => {
                            const isSelected = selectedKeys.includes(f.flow_key);
                            return (
                                <div
                                    key={f.flow_key}
                                    onClick={() => toggle(f.flow_key)}
                                    className={cn(
                                ...
                                >
                                    <div className="flex flex-col">
                                        <span className="text-xs truncate max-w-[240px]">{f.display_name}</span>
                                        <span className="text-[10px] text-muted-foreground font-mono">#{f.flow_key}</span>
                                    </div>
                    ...
                    {(content.quick_replies || []).map((qr, i) => (
                        <div key={i} className="flex gap-2 items-center">
                            ...
                            <div className="flex-1 space-y-1">
                                ...
                                <FlowSelect
                                    flows={flows}
                                    selectedKey={qr.payload || ''}
                                    onChange={key => updateQuickReply(i, { ...qr, payload: key })}
                        ...
                        <div key={i} className="flex gap-2 items-start bg-muted/20 p-2 rounded-lg border border-border/50">
                            <div className="flex-1 space-y-2">
                                ...
                                <div className="space-y-1">
                                    ...
                                    <FlowSelect
                                        flows={flows}
                                        selectedKey={btn.payload || ''}
                                        onChange={key => updateButton(i, { ...btn, payload: key })}
                                        placeholder="Chọn Flow hoặc nhập Payload..."
                                    />
                                </div>
                            </div>
